...
header-includes:
  - |
    ...
    \usepackage{fancyhdr}
    \usepackage{eso-pic}  % allows overlaying elements on a page

    % Keep your usual fancy layout for all pages
    \pagestyle{fancy}
    \fancyhf{} % no footer on normal pages

    % Add "Updated \today" footer on the last page only
    \AtEndDocument{%
      \AddToShipoutPictureBG*{%
        \AtPageLowerLeft{%
          \raisebox{64pt}[0pt][0pt]{%
            \makebox[\paperwidth][c]{\textcolor{gray}{\textit{Updated \today}}}%
          }%
        }%
      }%
    }
output: 
  vitae::moderncv:
    theme: banking
    # Replaces true endashes with a single dash
    pandoc_args: ["--lua-filter=replace_dash.lua"]
---
...
# First, overwrite the file (create/empty)
cat("", file = OUTFILE)
...
for (doi in DOI) {
    ...
    # Append each new entry (after emptying the file initially)
